...
class DLRM_Embed(nn.Module):
    def __init__(self, emb_l):
        super().__init__()
        self.emb_l = emb_l

    def forward(self, *emb_args):
        ly = DLRM_Net.apply_emb(self, self.emb_l, *emb_args)
        return torch.stack(ly)


class DLRM_Top(nn.Module):
    def __init__(self, top_l, loss_threshold):
        super().__init__()
        self.top_l = top_l
        self.loss_threshold = loss_threshold

    def forward(self, x, ly):
        t = [x] + list(torch.unbind(ly))
        z = ipex.nn.functional.interaction(*t)
        p = self.top_l(z)
        if 0.0 < self.loss_threshold and self.loss_threshold < 1.0:
            z = torch.clamp(
                p, min=self.loss_threshold, max=(1.0 - self.loss_threshold)
...
class DLRM_HT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.numel = 0
        self.bot_l = DLRM_Net.create_mlp(self, ln_bot, sigmoid_bot)
        emb_l = DLRM_Net.create_emb(self, m_spa, ln_emb, None, np_init_emb_weight)
        self.emb_l = DLRM_Embed(emb_l)
        top_l = DLRM_Net.create_mlp(self, ln_top, sigmoid_top)
        self.top_l = DLRM_Top(top_l, loss_threshold)

    def forward(self, dense_x, *emb_args):
        x = torch.jit._fork(self.bot_l, dense_x)
        ly = self.emb_l(*emb_args)
        x = torch.jit._wait(x)
        return x, ly
        #return self.top_l(x, ly)
...
def run():
    ...
    parser.add_argument("--ht", action="store_true", default=False)
    ...
    torch.set_num_interop_threads((args.share_weight_instance * 2) if args.ht else 1)
    ...
    Net = DLRM_HT if args.ht else DLRM_Net
    dlrm = Net(
        m_spa,
        ln_emb,
        ln_bot,
